...
| RENAME source.ip AS entity_name
| LOOKUP JOIN lookup-critical-assets ON entity_name
| RENAME entity_name AS source.ip
| EVAL
...
Filters to external IPs only (excludes RFC 1918). Excludes email-domain alerts because `source.ip` in email logs = sending mail server, not the attacker. Domain-count multiplier is higher (0.30) because external IPs across multiple domains is a stronger signal. An optional LOOKUP JOIN against `lookup-critical-assets` applies a criticality multiplier (1.5x for critical assets, 1.2x for high) to amplify risk scores for high-value IP entities. If the LOOKUP JOIN is unavailable, remove that block — the rule functions identically but without criticality weighting.
...
- **Optional**: CDN/scanner IP exclusion lookup for false positive reduction.
